...
public class File_operator {
	...
	ArrayList<CFG> parse_byte_code(File file) {
		...
		while (scan.hasNextLine()) {
			...
			if (state == state_machine.function) {
				...
				String bc_line_no;
				if (firstChar < str.length() && firstChar - 2 > -1) {
					// System.out.println("Str: "+str);
					bc_line_no = str.substring(0, firstChar - 2);
					// System.out.println("bc_line_no: "+bc_line_no);
					Instruction inst = l.addInstruction(str
							.substring(firstChar));
	 ...
	 * @param file
	 */
	ArrayList<BasicBlock> getBasicBlocks(ArrayList<Instruction> instructions,
			ArrayList<Instruction> leaders) {
		ArrayList<BasicBlock> basicBlocks = new ArrayList<BasicBlock>();
		// List of leaders of the program
		int i = 1;
		BasicBlock b = new BasicBlock();
		Instruction currentLeader = leaders.get(0);
	...
	ArrayList<Instruction> getLeaders(Function f) {
		...
		for (Line l : f.getLines()) {
			...
			for (Instruction inst : l.getInstructions()) {
				...
				if (i == 0) {
					// Check if the current instruction is already in the
					// leaders list
				...
				if (inst.type == inst.control_flow) {
					...
					if (!inst.getInstruction_text().equals("Unwind")
						...
						int destination = Integer.parseInt(inst.getBCLineNO())
								+ offset;
						Instruction dest_instruction = getDestinationInstruction(
								destination, f);
						inst.setDestination(dest_instruction);
						// Check if the leader to be added is already contained
						// in
						// the leaders list
				...
				//
				// }
				// If the instruction is following a conditional branch
				// instruction
				else if (followingConditional) {
					// Check if the leader to be added is already contained in
					// the leaders list
					if (!leaders.contains(inst))
						leaders.add(inst);
					followingConditional = false;
	...
	public CFG getCFG(ArrayList<BasicBlock> basicBlocks) {
		...
		for (int i = 0; i < cfg.getNodes().size(); i++) {
			...
			if (bi.getInstructions().size() == 1)
				...
				last_instr = bi.getInstructions().get(
						bi.getInstructions().size() - 1);
			else
				continue;
	 ...
	 * 
	 * @param destination
	 *            the bytecode line number of destination
	 * @param func
